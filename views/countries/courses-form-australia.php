...
<body>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 animate-box">
                                ...
                                if ($_SERVER["REQUEST_METHOD"] == "POST") {
                                    ...
                                    // $selectedUniversity = isset($_POST['selectedUniversity']) ? $_POST['selectedUniversity'] : '';
                                    ...
                                    if (empty($fullname) || empty($email) || empty($message) || empty($selectedUniversity) || empty($selectedQualification) || empty($selectedCourse)) {
                                        echo '<p style="color: red;">Please fill all required fields.</p>';
                                    } else {
                                        $database = new Database();
                                        $connection = $database->connection;

                                        $sql = "INSERT INTO form2_courses (fullname, email, message, selectedUniversity, selectedQualification, selectedCourse) VALUES (?, ?, ?, ?, ?, ?)";
                                        $stmt = $connection->prepare($sql);
                                        $stmt->bind_param("ssssss", $fullname, $email, $message, $selectedUniversity, $selectedQualification, $selectedCourse);
                                ...
                                <form method="POST" action="courses-form2.php" autocomplete="off">
                                    <input type="hidden" name="selectedUniversity" id="selectedUniversity" class="form-control" placeholder="Selected University" required>
                                    ...
                                    <input type="hidden" name="selectedCourse" id="selectedCourse" class="form-control" placeholder="Selected Course" required>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Fetch and display the values from local storage
            // var selectedUniversity = localStorage.getItem("universitySelect");
            var selectedQualification = localStorage.getItem("qualificationSelect");
            var selectedCourse = localStorage.getItem("courseSelect");
            ...
            if (selectedUniversity !== null) {
                document.getElementById('selectedUniversity').value = selectedUniversity;
            }
            ...
            // Clear the selections in local storage
