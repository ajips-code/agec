...
$albumname = $database->sanitize($_POST['moreaname']);
$albumname = strtolower($albumname);
$albumid = $database->sanitize($_POST['mid']);
...
if (!file_exists('images/photoalbum/' . $albumname)) {
	...
	for ($i = 0; $i < count($_FILES["morealbumpic"]["name"]); $i++) {
		$target_dir = "images/photoalbum/$albumname/";
		$image = $_FILES['morealbumpic']['name'][$i];
		$target_file = $target_dir . basename($_FILES["morealbumpic"]["name"][$i]);
		$uploadOk = 1;
		$imageFileType = strtolower(pathinfo($target_file, PATHINFO_EXTENSION));
		// Check if image file is a actual image or fake image
		$check = getimagesize($_FILES["morealbumpic"]["tmp_name"][$i]);
		if ($check !== false) {
		...
		// Check file size
		if ($_FILES["morealbumpic"]["size"][$i] > 9000000) {
		...
		if ($uploadOk == 0) {
			...
			if (move_uploaded_file($_FILES["morealbumpic"]["tmp_name"][$i], $target_file)) {
				$insert = "INSERT INTO images(album_id,picture,datetime)VALUES('$albumid','$image',NOW())";
